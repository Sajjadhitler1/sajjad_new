local function callback_reply(extra, success, result)
	...
	if access == 1 then
		if result.from.phone then
			...
			if string.sub(result.from.phone, 0,2) == '98' then
				number = number.."\nState : Islamic Republic Of Iran"
	...
	.."Groups Name : "..string.gsub(result.to.print_name, "_", " ").."\n"
	.."Groups ID : "..result.to.id
	send_large_msg(org_chat_id, info)
end

local function callback_res(extra, success, result)
	if success == 0 then
		return send_large_msg(org_chat_id, "Entered Username Is Wrong")
	end
	--icon & rank ------------------------------------------------------------------------------------------------
...
local function run(msg, matches)
	...
	if matches[1] == '/infodel' and is_sudo(msg) then
		...
		return 'Demoted From His/Her Rank'
	...
	elseif #matches == 2 then
		...
		if not msg.reply_id then
			...
			--info ------------------------------------------------------------------------------------------------
	info = "Full Name : "..string.gsub(msg.from.print_name, "_", " ").."\n"
	...
	.."Connected Device : "..hardware.."\n"
	.."Messages : "..user_info.msgs.."\n"
	.."Groups Name : "..string.gsub(msg.to.print_name, "_", " ").."\n"
	.."Groups ID : "..msg.to.id
